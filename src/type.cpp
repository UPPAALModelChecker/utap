...
kind_t type_t::getKind() const { return unknown() ? UNKNOWN : data->kind; }
...
bool type_t::isPrefix() const
{
    ...
    case Constants::EXTERNAL_FUNCTION:
...
bool type_t::unknown() const { return data == NULL || data->kind == UNKNOWN; }
...
bool type_t::is(kind_t kind) const
{
    ...
    return (getKind() == kind) || (isPrefix() && get(0).is(kind)) || (getKind() == RANGE && get(0).is(kind)) ||
           (getKind() == REF && get(0).is(kind)) || (getKind() == LABEL && get(0).is(kind));
}
...
position_t type_t::getPosition() const { return data->position; }

bool type_t::isIntegral() const
{
    return is(Constants::INT) || is(Constants::BOOL) || is(PROCESSVAR) || is(LOCATION) || is(LOCATION_EXPR);
}

bool type_t::isInvariant() const { return is(INVARIANT) || isIntegral(); }
...
bool type_t::isFormula() const { return is(FORMULA) || isConstraint(); }
...
bool type_t::isConstant() const
{
    ...
    case RECORD:
        ...
        return true;
    default: return size() > 0 && get(0).isConstant();
    }
}
...
bool type_t::isNonConstant() const
{
    ...
    case RECORD:
        ...
        return true;
    default: return size() == 0 || get(0).isNonConstant();
    }
}

type_t type_t::createRange(type_t type, expression_t lower, expression_t upper, position_t pos)
{
...
type_t type_t::createRecord(const vector<type_t>& types, const vector<string>& labels, position_t pos)
{
...
type_t type_t::createFunction(type_t ret, const std::vector<type_t>& parameters, const std::vector<std::string>& labels,
                              position_t pos)
{
...
type_t type_t::createExternalFunction(type_t ret, const std::vector<type_t>& parameters,
                                      const std::vector<std::string>& labels, position_t pos)
{
    assert(parameters.size() == labels.size());
    type_t type(EXTERNAL_FUNCTION, pos, parameters.size() + 1);
    type.data->children[0].child = ret;
    for (size_t i = 0; i < parameters.size(); i++) {
        type.data->children[i + 1].child = parameters[i];
        type.data->children[i + 1].label = labels[i];
...
type_t type_t::createPrimitive(kind_t kind, position_t pos) { return type_t(kind, pos, 0); }
...
string type_t::toString() const
{
    ...
    switch (getKind()) {
    case Constants::UNKNOWN: kind = "unknown"; break;

    case Constants::RANGE: kind = "range"; break;

    case Constants::ARRAY: kind = "array"; break;
    ...
    case Constants::COMMITTED: kind = "committed"; break;

    case Constants::BROADCAST: kind = "broadcast"; break;

    case Constants::VOID_TYPE: kind = "void"; break;

    case Constants::CLOCK: kind = "clock"; break;

    case Constants::FRACTION: kind = "fraction"; break;

    case Constants::INT: kind = "int"; break;

    case Constants::DOUBLE: kind = "double"; break;

    case Constants::STRING: kind = "string"; break;

    case Constants::BOOL: kind = "bool"; break;

    case Constants::SCALAR: kind = "scalar"; break;

    case Constants::CHANNEL: kind = "channel"; break;

    case Constants::INVARIANT: kind = "invariant"; break;

    case Constants::GUARD: kind = "guard"; break;

    case Constants::DIFF: kind = "diff"; break;

    case Constants::CONSTRAINT: kind = "constraint"; break;
    ...
    case Constants::PROCESS: kind = "process"; break;

    case Constants::INSTANCE: kind = "instance"; break;

    case Constants::LABEL: kind = "label"; break;
    case Constants::EXTERNAL_FUNCTION: kind = "external_function"; break;
    case Constants::FUNCTION: kind = "function"; break;

    case Constants::LOCATION_EXPR:
    case Constants::LOCATION: kind = "location"; break;

    case Constants::BRANCHPOINT: kind = "branchpoint"; break;

    // LSC
    case INSTANCELINE: kind = "instance line"; break;
    case MESSAGE: kind = "message"; break;
    case CONDITION: kind = "condition"; break;
...
string type_t::toDeclarationString() const
{
    ...
    case Constants::CHANNEL: kind = "chan"; break;
    ...
    case Constants::LABEL: label = true; break;
