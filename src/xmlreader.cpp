...
#include "tags.cc"
    ...
    /** Parse optional winning tag. */
    bool XMLReader::winning()
    {
	if (begin(TAG_WINNING, false)) 
	{
	    read();
	    return true;
	}
    ...
    /** Parse optional losing tag. */
    bool XMLReader::losing()
    {
	if (begin(TAG_LOSING, false)) 
	{
	    read();
	    return true;
	}
	...
	if (begin(TAG_LOCATION, false))
	{
		...
		/* Is the location winning or losing?
		 */
		l_winning = winning();
		l_losing = losing();
		...
		if (l_winning)
		{
		    parser->procStateWinning(l_name.c_str());
		}
		if (l_losing)
		{
		    parser->procStateLosing(l_name.c_str());
		}
	    }
